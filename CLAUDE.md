...
**Recent changes:**
- **2026-09-08**: A step that reports its own failure now marks the workflow FAILED - `StepFailure` return contract, one shared `_finalize()` for `run()`/`resume()`, `WorkflowState.error` echoes the real step errors, and a failed resume is archived at all (#73)
