...
**Recent changes:**
- **2026-09-11**: `backup_db.sh` distinguishes "could not check Docker" from "container is not running" - the `docker ps | grep -q` pipeline replaced by a captured query so docker's own message is surfaced, an exit-code contract (`2` = never established / `unknown`, `3` = checked and absent, `1` stays generic), whole-line literal name matching, and `status` reporting partial results instead of aborting (#93)
