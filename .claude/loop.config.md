...
|-----------|-------|-------|
| `BACKLOG_SOURCE` | GitHub milestone **`silent-success`** | `gh issue list --milestone silent-success --state open` **is** the roster — do not write the answer down beside it. This row carried a fixed enumeration ("Ten rows: #71 … #73–#80") from 2026-09-06 until 2026-09-17; by then it named a closed issue as the live instance and omitted most of the backlog, because the milestone grows every time an iteration files deferrals. **One row is never work:** #72, the epic tracker, carried terminal per §3. ⚠ **This milestone is one of two carriers of the roster, and they drift.** An issue is selectable only with *both* the milestone and a row in `LEDGER_ROOT/silent-success/queue.md`, and nothing enforces the pair: #99–#105 (deferrals filed during #71) and #107 (filed during #73) had neither until a curation pass on 2026-09-17 rostered them. Reconcile in **both** directions at each curation pass; a mismatch is a finding, not a cue to rebuild one carrier from the other. |
...
| `PRIORITY_LABELS` | ⚠ **the `priority:*` labels do not order this backlog.** Selection order is the **implementation-order table on #72**, as amended — a later comment may revise it and **the most recent amendment wins**. Read it at selection; do not cache it here. Then `Depends on:`/`Blocked by`, tiebreak issue-number ascending | The labels `priority:high\|medium\|low` exist in the repo and are set on some milestone rows, but they describe an issue rather than its position, and they disagree with the table — a `priority:low` row can sit ahead of a `priority:high` one where the order is carrying a dependency. The engine's selection step assumes a *label* ordering; this backlog encodes it in an epic comment instead. Treat the table as authoritative, and re-read it rather than an earlier copy: it is human-owned, amended by comment, and an amendment renumbers the whole table rather than appending to it. |
